...
module Home = {
  module TransactionRow = {
    type kind =
      | Deposit
      | Withdrawal
      | TransferReceived({from_wallet: string})
      | TransferSent({to_wallet: string});
    ...
    let render = transaction => {
      ...
      let (title, nameOpt) =
        ...
        | Deposit => (txt("Dinheiro depositado"), None)
        | Withdrawal => (txt("Dinheiro sacado"), None)
        | TransferReceived({from_wallet}) => (
            txt("Transferência recebida"),
            Some(txt(from_wallet)),
          )
        | TransferSent({to_wallet}) => (
            txt("Transferência enviada"),
            Some(txt(to_wallet)),
          )
        };
  ...
  type input = {
    ...
    income: Amount.t,
    expenses: Amount.t,
    transactions: list(TransactionRow.transaction),
  };
  ...
  let actionBox = (~action, ~icon) =>
    <div
      ...
      <span className="pt-4 text-2xl text-grey-100">
        {Html.txt(action)}
      </span>
